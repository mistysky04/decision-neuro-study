...
% Default values
center        = [round(rect(3)/2) round(rect(4)/2)];
lineLength    = 10;
width         = 3;
scalaLength   = 0.9;
scalaPosition = 0.8;
...
while answer == 0
    ...
    %HEDIE: I changed scalaPosition - 0.16 to 1.30, im trying to make the
    %text go to the very top of the screen above the food
    DrawFormattedText(screenPointer, question, 'center', rect(4)*(scalaPosition - 0.70), [255 255 255]); 
    
    % Drawing the anchors of the scale as text
    if length(anchors) == 2
        % Only left and right anchors
        %HEDIE: Changed scalaPosition from +40 to +80, trying to lower for
        %full screen
        DrawFormattedText(screenPointer, anchors{1}, leftTick(1, 1) - textBounds(1, 3)/2,  rect(4)*scalaPosition+80, [],10,[],[],[],[],[]); % Left point
        DrawFormattedText(screenPointer, anchors{2}, rightTick(1, 1) - textBounds(2, 3)/2,  rect(4)*scalaPosition+80, [],10,[],[],[],[],[]); % Right point
    else 
